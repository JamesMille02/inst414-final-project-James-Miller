...
def add_awards_to_media_ratings():
    """Adds columns for nominations and wins to the media ratings based on Golden Globes awards data."""
    
    # File paths
    media_ratings_csv = 'data/processed/merged_media_ratings.csv'
    awards_csv = 'data/extracted/golden_globe_awards.csv'
    output_csv = 'data/processed/merged_media_ratings_with_awards.csv'
    ...
    merged_df = pd.merge(media_ratings_df, nominations_wins, left_on='media', right_on='film', how='left')
    
    # Drop the extra 'film' column after merging
    merged_df.drop(columns=['film'], inplace=True, errors='ignore')
    
    # Fill NaN values with 0 for films not found in the awards data
    merged_df['nominations'] = merged_df['nominations'].fillna(0)
    merged_df['wins'] = merged_df['wins'].fillna(0)
    
    # Rename columns
    merged_df.rename(columns={'wins': 'golden_globe_wins', 'nominations': 'golden_globe_nominations'}, inplace=True)
